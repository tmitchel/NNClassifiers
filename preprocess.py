...
def main(args):
    ...
    for ifile in input_files:
        ...
        big_boi = pd.concat([big_boi, input_data])
        selection_boi = pd.concat([selection_boi, selection_data])
        names = np.append(names, new_name)
        isSignal = np.append(isSignal, sig)
        weight_boi = np.append(weight_boi, weight)
