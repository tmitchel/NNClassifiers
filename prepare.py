...
def build_filelist(epath, mpath):
    ...
    nominal = {idir: [ifile.split('/')[-1]
               ...
               for idir in directories if idir.split('/')[-1] == 'NOMINAL'}

    systematics = {idir: [ifile.split('/')[-1]
                          for ifile in glob('{}/merged/*.root'.format(idir))]
                   for idir in directories if idir.split('/')[-1] != 'NOMINAL'}
...
def loadFile(path, ifile, open_file, syst='nominal'):
    ...
    if 'mutau' in path:
        ...
        raise Exception(
            'Input files must have MUTAU or ETAU in the provided path. You gave {}, ya goober.'.format(path))
    ...
    return {
        ...
        'weights': weights,
        'index': index,
        'somenames': np.full(len(slim_df), ifile),
        'lepton': np.full(len(slim_df), channel),
    }, syst.replace(';1', '')


def handle_file(all_data, idir, ifile):
    default_object = {
    ...
    full_path = '{}/merged/{}'.format(idir, ifile)
    open_file = uproot.open(full_path)
    syst = 'nominal' if 'NOMINAL' in idir else idir.split('SYST_')[-1]
    filename = ifile.replace('.root', '')

    proc_file, syst = loadFile(full_path, filename, open_file, syst)
...
def main(args):
    ...
    for idir, files in nominal.iteritems():
        all_data = {}
        for ifile in files:
            all_data = handle_file(all_data, idir, ifile)
    ...
    for idir, files in systematics.iteritems():
        ...
        dir = idir.split('SYST_')[-1]
        store[dir] = format_for_store(all_data, dir, scaler)
